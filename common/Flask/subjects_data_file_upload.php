<?php
   if(isset($_FILES['excel_file']) && isset($_POST['submit']))
   {
	  ...
	  try
	  {
		  if($file_name=='subjects_data.xlsx')
		  {
			...
			if(in_array($upload_file_ext,$file_exts) && move_uploaded_file($file_tmp,"Excel File/".$file_name))
			{
			  ...
			  echo "<script>window.open('http://localhost:5000/Subjects_Excel_File_Format_Checking','_self');</script>";
...
<body>
   <center>
      ...
      <div style="padding: 20px;border:1px solid black" id='two'>
         ...
         <ul align='left'>
            <li>The name of the file must be 'subjects_data'</li>
            <li>The file extension should be in .xlsx format</li>
            <li>For Reference File Format <a href="Excel File/File Formats/Subjects_File_Format.xlsx" 
               download='subjects_data.xlsx'>Click Here</a>
            </li>
         </ul>
      </div>
   </center>
